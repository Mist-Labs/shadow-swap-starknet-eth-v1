...
use shadow_swap::shadow_swap_settlement::{
    IERC20Dispatcher, IERC20DispatcherTrait, IShadowSettlementDispatcher,
    IShadowSettlementDispatcherTrait, IntentDetail, IntentPublic, ProcessReason, RemoteRootSnapshot,
    ShadowSettlement,
};
use snforge_std::{
...
) -> IShadowSettlementDispatcher {
    let contract = declare("ShadowSettlement").unwrap().contract_class();
    let mut calldata = array![];
    owner.serialize(ref calldata);
    relayer.serialize(ref calldata);
    // avnu_exchange: zero address on testnets — set via set_avnu_exchange before swap tests
    contract_address_const::<0>().serialize(ref calldata);
...
fn fill_batch_with_filler(
    dispatcher: IShadowSettlementDispatcher, relayer: ContractAddress, start_index: u64, count: u64,
) {
...
#[test]
fn test_constructor_avnu_exchange_readable() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let dispatcher = deploy_settlement(owner, relayer);

    let exchange = dispatcher.get_avnu_exchange();
    assert!(exchange == contract_address_const::<0>(), "Exchange should be zero initially");
}
...
// ===== AVNU EXCHANGE ADMIN TESTS =====

#[test]
fn test_set_avnu_exchange_success() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let exchange = contract_address_const::<'avnu'>();
    let dispatcher = deploy_settlement(owner, relayer);

    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.set_avnu_exchange(exchange);
    stop_cheat_caller_address(dispatcher.contract_address);

    assert!(dispatcher.get_avnu_exchange() == exchange, "Exchange should be updated");
}

#[test]
#[should_panic(expected: "Not owner")]
fn test_set_avnu_exchange_reverts_if_not_owner() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let exchange = contract_address_const::<'avnu'>();
    let dispatcher = deploy_settlement(owner, relayer);
...
#[test]
#[should_panic(expected: "Invalid exchange address")]
fn test_set_avnu_exchange_reverts_if_zero() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let dispatcher = deploy_settlement(owner, relayer);

    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.set_avnu_exchange(contract_address_const::<0>());
}
        ...
        .assert_emitted(
            @array![
                (
                    ...
                    ShadowSettlement::Event::CommitmentAdded(
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::IntentMarkedSettled(
                        ShadowSettlement::IntentMarkedSettled {
                            nullifier_hash: nullifier, commitment, timestamp: 2000,
                        },
                    ),
                ),
            ],
...
fn test_sync_merkle_root_success() {
    ...
    assert!(
        u256 { low: snapshot.leaf_count_low, high: snapshot.leaf_count_high } == leaf_count,
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::RemoteRootSynced(
                        ShadowSettlement::RemoteRootSynced {
                            chain_id, root, leaf_count, snapshot_index: 0_u256,
                        },
                    ),
                ),
            ],
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::RemoteRootVerified(
                        ShadowSettlement::RemoteRootVerified {
                            chain_id, snapshot_index: 0_u256, verifier: relayer,
                        },
                    ),
                ),
            ],
...
fn test_settle_and_release_event_does_not_include_recipient() {
    ...
    // Event must NOT include recipient (privacy)
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::IntentSettled(
...
#[test]
#[should_panic(expected: "Dest token not whitelisted")]
fn test_settle_and_release_reverts_if_dest_token_not_whitelisted() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let user = contract_address_const::<'user'>();
    let dispatcher = deploy_settlement(owner, relayer);
    ...
    // Whitelist STRK but not USDC
    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.set_token_whitelist(strk_token.contract_address, true);
    stop_cheat_caller_address(dispatcher.contract_address);

    start_cheat_caller_address(strk_token.contract_address, owner);
    strk_token.transfer(dispatcher.contract_address, 1000_u256);
    ...
    dispatcher
        .settle_and_release(
            ...
            1000_u256,
            usdc_token, // dest_token: USDC (not whitelisted)
            1000_u256, // expected_dest_amount: quote amount
            900_u256, // min_dest_amount: slippage floor
            array![] // empty routes — will fail at whitelist check first
        );
}

#[test]
#[should_panic(expected: "min_dest_amount required for swap")]
fn test_settle_and_release_reverts_if_swap_without_min_amount() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let user = contract_address_const::<'user'>();
    let dispatcher = deploy_settlement(owner, relayer);
    let strk_token = deploy_mock_token(owner);
    let usdc_token = deploy_mock_token(owner);

    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.set_token_whitelist(strk_token.contract_address, true);
    dispatcher.set_token_whitelist(usdc_token.contract_address, true);
    stop_cheat_caller_address(dispatcher.contract_address);
    ...
    dispatcher
        .settle_and_release(
            ...
            0_u256, // expected_dest_amount: 0 — triggers min_dest_amount > 0 guard first
            0_u256, // min_dest_amount: 0 — must be > 0 for swaps
            array![],
        );
}

#[test]
#[should_panic(expected: "Routes required for swap")]
fn test_settle_and_release_reverts_if_swap_without_routes() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let user = contract_address_const::<'user'>();
    let dispatcher = deploy_settlement(owner, relayer);
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::TokenWhitelistUpdated(
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::Paused(ShadowSettlement::Paused {}),
                ),
            ],
        );
}
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::BatchConfigUpdated(
                        ShadowSettlement::BatchConfigUpdated {
                            new_batch_size: 20, new_timeout: 60,
                        },
                    ),
                ),
            ],
...
#[test]
#[should_panic(expected: "Invalid intent id")]
fn test_settle_and_release_reverts_if_zero_intent_id() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let user = contract_address_const::<'user'>();
    let dispatcher = deploy_settlement(owner, relayer);
    ...
    dispatcher
        .settle_and_release(
            0, // intent_id: zero — must be rejected
            0xabc,
            user,
            token.contract_address,
            500_u256,
...
#[test]
#[should_panic(expected: "Invalid nullifier hash")]
fn test_settle_and_release_reverts_if_zero_nullifier_hash() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let user = contract_address_const::<'user'>();
    let dispatcher = deploy_settlement(owner, relayer);
    ...
    dispatcher
        .settle_and_release(
            0x123,
            0, // nullifier_hash: zero — must be rejected
            user,
            token.contract_address,
            500_u256,
            contract_address_const::<0>(),
...
#[test]
#[should_panic(expected: "Invalid nullifier hash")]
fn test_mark_settled_reverts_if_zero_nullifier_hash() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let dispatcher = deploy_settlement(owner, relayer);

    add_and_process_commitment(dispatcher, relayer, 0x123, 0xabc, 0, 1900);

    start_cheat_caller_address(dispatcher.contract_address, relayer);
    dispatcher.mark_settled(0x123, 0); // zero nullifier — must be rejected
}

#[test]
#[should_panic(expected: "expected_dest_amount < min_dest_amount")]
fn test_settle_and_release_reverts_if_expected_less_than_min() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let user = contract_address_const::<'user'>();
    let dispatcher = deploy_settlement(owner, relayer);
    ...
    dispatcher
        .settle_and_release(
            ...
            1000_u256,
            usdc_token.contract_address,
            800_u256, // expected_dest_amount: less than min — must be rejected
            900_u256, // min_dest_amount: higher than expected
            array![] // guard fires before routes are checked
        );
}
...
fn test_propose_owner_success() {
    ...
    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.propose_owner(new_owner);
    stop_cheat_caller_address(dispatcher.contract_address);

    assert!(dispatcher.get_pending_owner() == new_owner, "Pending owner should be set");
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::OwnershipProposed(
                        ShadowSettlement::OwnershipProposed {
                            current_owner: owner, proposed_owner: new_owner,
                        },
                    ),
                ),
            ],
...
fn test_accept_ownership_success() {
    ...
    assert!(
        dispatcher.get_pending_owner() == contract_address_const::<0>(),
        "Pending owner should be cleared",
    );

    // New owner can now perform owner-only actions
    start_cheat_caller_address(dispatcher.contract_address, new_owner);
    dispatcher.set_relayer_status(relayer, false);
    stop_cheat_caller_address(dispatcher.contract_address);

    assert!(!dispatcher.is_relayer_authorized(relayer), "New owner should have authority");
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::OwnershipTransferred(
                        ShadowSettlement::OwnershipTransferred { previous_owner: owner, new_owner },
                    ),
                ),
            ],
        );
...
#[test]
fn test_cancel_ownership_proposal_success() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let new_owner = contract_address_const::<'new_owner'>();
    let dispatcher = deploy_settlement(owner, relayer);
    ...
    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.propose_owner(new_owner);
    dispatcher.cancel_ownership_proposal();
    stop_cheat_caller_address(dispatcher.contract_address);
    ...
    spy
        .assert_emitted(
            @array![
                (
                    dispatcher.contract_address,
                    ShadowSettlement::Event::OwnershipProposalCancelled(
                        ShadowSettlement::OwnershipProposalCancelled {
                            cancelled_proposed_owner: new_owner,
                        },
                    ),
                ),
            ],
...
#[test]
#[should_panic(expected: "Not owner")]
fn test_propose_owner_reverts_if_not_owner() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let new_owner = contract_address_const::<'new_owner'>();
    let dispatcher = deploy_settlement(owner, relayer);
...
#[test]
#[should_panic(expected: "Already owner")]
fn test_propose_owner_reverts_if_proposing_current_owner() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let dispatcher = deploy_settlement(owner, relayer);

    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.propose_owner(owner);
}

#[test]
#[should_panic(expected: "Invalid proposed owner")]
fn test_propose_owner_reverts_if_zero_address() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let dispatcher = deploy_settlement(owner, relayer);

    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.propose_owner(contract_address_const::<0>());
}

#[test]
#[should_panic(expected: "Not pending owner")]
fn test_accept_ownership_reverts_if_not_pending_owner() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let new_owner = contract_address_const::<'new_owner'>();
    let impostor = contract_address_const::<'impostor'>();
    let dispatcher = deploy_settlement(owner, relayer);
    ...
    start_cheat_caller_address(dispatcher.contract_address, impostor);
    dispatcher.accept_ownership();
}

#[test]
#[should_panic(expected: "No ownership proposal")]
fn test_accept_ownership_reverts_if_no_proposal() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let dispatcher = deploy_settlement(owner, relayer);
...
#[test]
#[should_panic(expected: "No ownership proposal")]
fn test_cancel_ownership_proposal_reverts_if_no_proposal() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let dispatcher = deploy_settlement(owner, relayer);
...
#[test]
fn test_propose_owner_overwrites_previous_proposal() {
    let owner = contract_address_const::<'owner'>();
    let relayer = contract_address_const::<'relayer'>();
    let new_owner_1 = contract_address_const::<'new_owner_1'>();
    let new_owner_2 = contract_address_const::<'new_owner_2'>();
    let dispatcher = deploy_settlement(owner, relayer);

    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.propose_owner(new_owner_1);
    dispatcher.propose_owner(new_owner_2); // overwrite
    stop_cheat_caller_address(dispatcher.contract_address);

    assert!(dispatcher.get_pending_owner() == new_owner_2, "Second proposal should win");
}
...
fn test_old_owner_loses_authority_after_transfer() {
    ...
    // Old owner tries an admin action — must be rejected
    start_cheat_caller_address(dispatcher.contract_address, owner);
    dispatcher.pause();
}
