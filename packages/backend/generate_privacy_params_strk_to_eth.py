#!/usr/bin/env python3
"""
ShadowSwap: 7.5 STRK (StarkNet) → ETH (Ethereum Mainnet)
...
ETH_SETTLEMENT     = os.environ['EVM_SETTLEMENT_ADDRESS']   # Ethereum mainnet settlement contract
SN_ACCOUNT         = os.environ['STARKNET_ACCOUNT_ADDRESS']
HMAC_SECRET        = os.environ['HMAC_SECRET']
BACKEND            = "http://127.0.0.1:8080"

# ── Swap parameters ───────────────────────────────────────────────────────────
AMOUNT_STRK   = "7500000000000000000"   # 7.5 STRK (18 decimals)
STRK_TOKEN    = "0x04718f5a0Fc34cC1AF16A1cdee98fFB20C31f5cD61D6Ab07201858f4287c938D"
RECIPIENT_ETH = os.environ['EVM_ACCOUNT_ADDRESS']   # your Ethereum wallet

ETH_DEST_ASSET = "nep141:eth.omft.near"

print("=" * 70)
print("SHADOWSWAP — StarkNet STRK → Ethereum ETH")
print("=" * 70)

# ── Dry-run quote first to confirm route exists ───────────────────────────────
print("\nChecking route availability (dry run)...")
...
dry_body = {
    ...
    "originAsset": "nep141:starknet.omft.near",
    "destinationAsset": ETH_DEST_ASSET,
    "amount": AMOUNT_STRK,
    "recipient": ETH_SETTLEMENT,
    "refundTo": SN_ACCOUNT,
...
if not dry.ok:
    print(f"❌ Route not available: {dry.status_code}: {dry.text}")
    sys.exit(1)

dry_data = dry.json()
min_out = dry_data.get("quote", {}).get("minAmountOut", "?")
amount_out = dry_data.get("quote", {}).get("amountOut", "?")
print(f"✅ Route confirmed")
print(f"   estimated ETH out: {int(amount_out)/1e18:.6f} ETH" if amount_out != "?" else f"   amountOut: {amount_out}")
print(f"   min ETH out:       {int(min_out)/1e18:.6f} ETH" if min_out != "?" else f"   minAmountOut: {min_out}")
...
# ── 2. NEAR 1Click /v0/quote (live) ──────────────────────────────────────────
quote_body = {**dry_body, "dry": False}

print(f"\nCalling NEAR 1Click /v0/quote (live)...")
q = requests.post(
    "https://1click.chaindefuser.com/v0/quote",
    json=quote_body,
    headers={"Content-Type": "application/json", "Authorization": f"Bearer {NEAR_API_KEY}"},
